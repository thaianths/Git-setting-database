                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                                ...
                                                (row["type"].ToString() == "trigger"))
                                            {
                                                using (Helper _hp = new Helper())
                                                {
                                                    _hp.execNonquery(row["tableName"].ToString(), txtcustomer2.Text.ToLower());
                                                }
                                                //execute_stored_procedure_function_trigger(row["tableName"].ToString(), txtcustomer2.Text);
                                            }
                                            else
                                            {
                                                ps.StandardInput.WriteLine("pg_restore  -h " + ip_cnn_customer + "  -p  " + Port_cnn_customer + " -U " + Id_cnn_customer + " -v -d " + txtcustomer2.Text + " " + _dir + row["tablename"].ToString() + ".sql");
                                            }
                                        }
                                        ps.StandardInput.Flush();
