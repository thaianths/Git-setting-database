    ...
    public partial class Setting : Form, Icreate, Ivalidate
    {
        ...
        public void create()
        {
            try
            {
                using (Service sv = new Service())
                {
                    if (drgViewTables.Columns.Count < 2)
        ...
        private void comboBox1_SelectedValueChanged(object sender, EventArgs e)
        {
            try
            {
                using (Service sv = new Service())
                {
                    ...
                    chkAll.Checked.CheckUncheckAll(this.drgViewTables, this.drgViewTables.Rows.Count);
                    drgViewTables.ClearSelection();
                    sv.comboBox1_SelectedValueChanged(comboBox1,
                                                      drgViewTables,
                                                      GetConfigServer<List<ModelProduct>>(_WorkingDirectory + fileName + ".xml"),
                                                      _WorkingDirectory + @"\logError.txt");
                }
            }
            catch (Exception ex)
        ...
        private void chkAll_Click(object sender, EventArgs e)
        {
            ...
            if (chkAll.Checked)
            {
                true.CheckUncheckAll(this.drgViewTables, this.drgViewTables.Rows.Count);
            }
            else
            {

                false.CheckUncheckAll(this.drgViewTables, this.drgViewTables.Rows.Count);
